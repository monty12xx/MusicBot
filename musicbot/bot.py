...
class MusicBot(discord.Client):
    ...
    async def cmd_help(self, message):
        await self.send_message(message.channel, """{} Commands For `ShinyHeaven V9`.\n```diff\n

[ summon ]  ( Summons the bot into voice chat. )
[ play ]  ( song name or url , or playlist link )
[ add SongName/Link/PlaylistLink ]  ( supports YT, SoundCloud links and Playlists )
[ pause ]  ( Pauses the lpmusic playing )
[ resume ]  ( Resumes the music that was paused )
[ playing ]  ( Shows the current song playing. )
[ shuffle ]  ( To shuffle the SongList )
[ skip ]  ( Skips the song that's currently playing )
[ volume ]  ( Check the volumes )
[ volume 1-100 ]  ( Increases/Decreases volume on your pc or phone )
[ clear ]  ( Clears the playlist )
[ clean ]  ( Delete's all the messages the bot left behind. )```""".format(message.author.mention))
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        """
        %play <song name> or <playlist link>
        """
        ...
        if 'entries' in info:
            ...
            reply_text = ":notes: Enqueued **%s** `postion` %s"
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        busymsg = await self.safe_send_message(
            channel, "Adding  %s songs to playlist `queue`..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
        ...
        print("Queued {}/{} songs in {} seconds at {:.2f}s/song, {:+.2g}/song from expected ({}s)".format(
            songs_processed,
            num_songs,
            self._fixg(ttime),
            ttime / num_songs,
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        """
        {command_prefix}search <user input>
        """
